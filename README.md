...
本项目实现一个可复现的糖尿病早期风险预测系统，包含数据预处理、特征选择、模型训练、推理、int8 量化和实验报告材料。代码入口均包含异常处理和日志记录，日志输出到 `logs/`。
...
.
├── scripts/
│   ├── train.py          # 训练、调参、评估、Badcase 输出
│   ├── predict.py        # 普通模型和量化模型推理
│   └── quantize.py       # 训练 Logistic Regression 并导出 int8 权重
...
3. 将不同量纲数据统一处理：年龄使用中位数填充和标准化，类别症状使用众数填充和 One-Hot 编码。
4. 使用互信息 `mutual_info_classif` 选择 Top-K 特征，减少冗余和噪声。
5. 训练并比较 Logistic Regression、Random Forest、Gradient Boosting。
6. 使用 5 折 Stratified K-Fold 以 F1 作为主指标选择最优模型。
7. 输出测试集指标、特征重要性、Badcase、最优权重和量化权重。
...
- `models/logistic_model.joblib`：用于量化推理的 Logistic Regression 管线。
...
本项目代码为独立实现，未复制第三方项目代码。数据集引用 UCI Machine Learning Repository；模型算法基于 scikit-learn 官方库，包括 Logistic Regression、Random Forest、Gradient Boosting、OneHotEncoder、StandardScaler 和 mutual information 特征选择。实验报告中涉及数据集、论文算法或开源实现时应保留明确引用。
